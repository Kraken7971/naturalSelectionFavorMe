...
//						*** SHARING MECHANICS FOLLOW ***
		microbe* nearestA = this;
		microbe* nearestB = this;
		for(int i = -1 ; i <= 1 && flag; i++){
			for(int j = -1; j <= 1; j++){
				if(i==0 && j==0)
					continue;
				if( // Massive Condition for possibility of sharing
				world[defaultY+i][defaultX+j].lastMicrobe != NULL
				&&
				world[defaultY+i][defaultX+j].lastMicrobe->foodA > nearestA->foodA
				){
					nearestA = world[defaultY+i][defaultX+j].lastMicrobe;
				}
		  	}
		}
		
		if(nearest != this) {	// Residue Here.
			shareFood(*nearest);
		}
		// Food Consumption. Replace 1 by sustenence.
		foodA = foodA-1;
		foodB = foodB-1;
